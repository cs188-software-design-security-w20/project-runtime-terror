    ...
    render() {
        ...
        if (!Array.isArray(song_info)) {
            ...
            const song_cards = song_info.map((info, index) => <Grid.Column stretched key={index}><SongCard title={info.title} artist={info.artist} album={info.album} art_url={info.art_url}
                 url={info.url} create_url={info.create_url} _token={info._token} uri={info.uri} deviceid={info.deviceid} type={info.type} account_type={info.account_type} /></Grid.Column>)
...
export class SongInfo {
    ...
    constructor(title, artist, album, art_url, url, create_url, _token, uri, deviceid, type, account_type) {
        this.title = title
        this.artist = artist
        this.album = album
