...
class SongGrid extends Component {
    ...
    render() {
        ...
        if (!Array.isArray(song_info)) {
            ...
            const song_cards = song_info.map((info, index) => <Grid.Column stretched key={index}><SongCard title={info.title} artist={info.artist} album={info.album} art_url={info.art_url} rating={info.rating}/></Grid.Column>)
...
export class SongInfo {
    ...
    constructor(title, artist, album, art_url, rating) {
        this.title = title
        this.artist = artist
        this.album = album
